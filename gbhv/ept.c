...
NTSTATUS(*ObReferenceObjectByHandleWithTagOrig)(HANDLE Handle, ACCESS_MASK DesiredAccess, POBJECT_TYPE ObjectType, KPROCESSOR_MODE AccessMode,
	...
	ULONG Tag, PVOID* Object, POBJECT_HANDLE_INFORMATION HandleInformation)
{
	...
	if (strstr(process_name, "cheatengine"))
	{
		...
		return ObReferenceObjectByHandleWithTagOrig(Handle, 0, ObjectType, KernelMode, Tag, Object, HandleInformation);
	}
	return ObReferenceObjectByHandleWithTagOrig(Handle, DesiredAccess, ObjectType, AccessMode, Tag, Object, HandleInformation);
}
...
BOOL HvEptLogicalProcessorInitialize(PVMM_PROCESSOR_CONTEXT ProcessorContext)
{
	...
	if (!HvEptAddPageHook(ProcessorContext, (PVOID)ObReferenceObjectByHandleWithTag, (PVOID)ObReferenceObjectByHandleWithTagHook, (PVOID*)&ObReferenceObjectByHandleWithTagOrig))
	{
		HvUtilLogError("Failed to build page hook for ObReferenceObjectByHandleWithTag");
		HvEptFreeLogicalProcessorContext(ProcessorContext);
		return FALSE;
	}
	return TRUE;
